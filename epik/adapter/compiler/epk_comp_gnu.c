...
static int epk_open_exe()
{
...
#ifdef __APPLE__
   char path[2048];
   uint32_t sze = sizeof(path);
   if (_NSGetExecutablePath(path, &sze) == 0)
     BfdImage = bfd_openr(path, 0 );
   else
     elg_error_msg("File name too long. Buffer should be %u", sze);
#else
   ...
   epk_filter_add("__ompregion_*"); /* OpenMP "outlined" function */
#ifdef __ia64__
   /* workaround for failure to get correct function addresses */
   epk_filter_add("PAPI_*");
   epk_filter_add("_papi_*");
   epk_filter_add("pfm_*");
   epk_filter_add("pfmw_*");
   epk_filter_add("epik_*");
   epk_filter_add("ELG_*");
   epk_filter_add("Epk*");
   epk_filter_add("Esd*");
   epk_filter_add("Elg*");
 ...
 * The call is generated by the GNU compilers
 */
