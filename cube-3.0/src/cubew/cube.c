...
cube_metric* cube_def_met(cube_t* this, const char* disp_name, const char* uniq_name, const char* dtype, const char* uom,
  ...
  char* ndisp = cubew_strdup(disp_name);
  char* nuniq = cubew_strdup(uniq_name);
  char* ndtype = cubew_strdup(dtype);
  char* nuom = cubew_strdup(uom);
  char* nval = cubew_strdup(val);
  char* nurl = cubew_strdup(url);
  char* ndescr = cubew_strdup(descr);
...
cube_region* cube_def_region(cube_t* this, const char* name, long begln, long endln,
  ...
  char* nname = cubew_strdup(name);
  char* nurl = cubew_strdup(url);
  char* ndescr = cubew_strdup(descr);
  char* nmod = cubew_strdup(mod);
...
void cube_write_def(cube_t* this, FILE* fp) {
  ...
  for (i = 0; i < attr->size; i++) {
    ...
    char * _key  = (char *)services_escape_to_xml(key);
    char * _value  = (char *)services_escape_to_xml(value);
  ...
  for (i = 0; i < mirr->size; i++) {
    ...
    char * _mirror  = (char *)services_escape_to_xml(mirror);
