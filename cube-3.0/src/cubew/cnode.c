...
*/
void cube_cnode_init(cube_cnode* this, struct cube_region* callee, const char* mod, int line, cube_cnode* parent) {
  this->parent = parent;
  this->callee = callee;
...
void cube_cnode_writeXML(cube_cnode* this, FILE* fp) {
  ...
  char * _mod   = (char *)services_escape_to_xml(cube_cnode_get_mod(this));
  ...
  if (strcmp(cube_cnode_get_mod(this), "") != 0)
    fprintf(fp, "mod=\"%s\" ",  _mod ? _mod: cube_cnode_get_mod(this));
